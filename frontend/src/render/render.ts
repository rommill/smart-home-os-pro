...
function getLocalizedRoomName(room: RoomData, currentLang: string): string {
...
function createRoomCardHtml(
  ...
  const targetTemp =
    room.target_temperature !== undefined ? room.target_temperature : 23.0;
...
export function renderRooms(data: RoomData[]): void {
  ...
  data.forEach((room) => {
    const card = grid.querySelector(
      `.room-card[data-room-id="${room.id}"]`,
    ...
    const titleTextNode = card.querySelector(
      ".room-title-text",
    ) as HTMLElement | null;
    if (titleTextNode) {
    ...
    const updatedStaticLabel = card.querySelector(
      ".static-updated-label",
    ) as HTMLElement | null;
    ...
    const timeDisplay = card.querySelector(
      ".time-display",
    ) as HTMLElement | null;
    ...
    if (!card.classList.contains("is-active")) {
      const targetTextNode = card.querySelector(
        `#target-val-${room.id}`,
      ) as HTMLElement | null;
      const inputSlider = card.querySelector(
        `input[type="range"]`,
