...
import { animateCards } from "../theme/theme.js";
import { renderClimateControlPanel } from "./climate.js";
import { getRoomIcon, getTemperatureStyles, toggleAuthView, updateStatus, } from "./utils.js";
export { toggleAuthView, updateStatus };
function generateSystemBadgesHtml(currentTemp, targetTemp, isOffline, currentLang) {
    if (isOffline || !currentTemp)
        return "";
...
function createRoomCardHtml(room, currentLang, isFirstRender) {
    ...
    const isOffline = room.temperature === undefined ||
        room.temperature === null ||
        room.temperature === "N/A";
    const tempValue = isOffline ? "N/A" : room.temperature;
    const time = room.last_update && room.last_update !== "0001-01-01T00:00:00Z"
        ? new Date(room.last_update).toLocaleTimeString(currentLang === "ru" ? "ru-RU" : "et-EE")
        : t("noData");
    const targetTemp = room.target_temperature !== undefined ? room.target_temperature : 23.0;
    const styles = getTemperatureStyles(room.temperature, isOffline);
    if (!isOffline && room.temperature) {
        const currentFloat = parseFloat(room.temperature);
        const targetFloat = targetTemp;
...
export function renderRooms(data) {
    ...
    data.forEach((room) => {
        ...
        if (updatedStaticLabel)
            updatedStaticLabel.textContent = t("updated");
        ...
        if (acContainer) {
            acContainer.innerHTML = generateSystemBadgesHtml(room.temperature, targetTemp, isOffline, currentLang);
        }
        if (!card.classList.contains("is-active")) {
            const targetTextNode = card.querySelector(`#target-val-${room.id}`);
            const inputSlider = card.querySelector(`input[type="range"]`);
            if (targetTextNode)
                targetTextNode.textContent = `${targetTemp}°C`;
            if (inputSlider) {
                inputSlider.value = targetTemp.toString();
                inputSlider.setAttribute("value", targetTemp.toString());
            }
        }
    });
}
